...
bool Huanyu_start_object::ReadAndWriteLoopProcess()
{
	...
	while(ros::ok())
	{
		...
		this->sampleFreq_hz = 1.0f / this->dt;
...
void Huanyu_start_object::MahonyAHRSupdateIMU(float gx, float gy, float gz, float ax, float ay, float az)
{
	...
	if(!((ax == 0.0f) && (ay == 0.0f) && (az == 0.0f))) {
		...
		if(twoKi > 0.0f) {
			integralFBx += twoKi * halfex * this->dt; //(1.0f / this->sampleFreq_hz);	// integral error scaled by Ki
			integralFBy += twoKi * halfey * this->dt; //(1.0f / this->sampleFreq_hz);
			integralFBz += twoKi * halfez * this->dt; //(1.0f / this->sampleFreq_hz);
	...
	gx *= (0.5f * this->dt);		// pre-multiply common factors
	gy *= (0.5f * this->dt);
	gz *= (0.5f * this->dt);
